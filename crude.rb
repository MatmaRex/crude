...
class CrudeCompiler
	...
	def compile sexp
		...
		if self.respond_to? :"compile_#{type}"
			self.send :"compile_#{type}", rest
		else
			raise "unsupported node type `#{type}` / #{rest.inspect}"
		end
	end
	
	def compile_call rest
	...
	alias compile_attrasgn compile_call
	
	def compile_arglist rest
		out = rest.map{|a| compile a}.join ', '
		# p rest, out
		out
	end
	
	def compile_defn rest
	...
	def compile_str rest
		 # assumes all single-char strings to be chars, not char arrays
		this = rest[0]
		if this.length != 1
			this.inspect
		else
	...
	def compile_not rest
		this = rest[0]
		"!(#{compile this})"
	end
	
	def compile_and rest
	...
	def compile_return rest
		"return #{compile rest[0]};"
	end
	
	def compile_break rest
		"break;"
	end
	
	def compile_next rest
		"continue;"
	end
	
	def compile_lvar rest
		this = rest[0]
		this
	end
	
	def compile_block rest
		rest.map{|a|
	...
	alias compile_scope compile_block
	
	def compile_true rest
		"true"
	end
	
	def compile_false rest
		"false"
	end
	
	def compile_nil rest
		"null"
	end
